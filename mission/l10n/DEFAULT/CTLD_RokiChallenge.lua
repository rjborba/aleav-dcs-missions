...
ctld.JTAC_sendNotificationMessage = false -- enables or disables notifications for new targets
...
ctld.pickupZones = {
    { "BLUE_FARP_PICKUP_ZONE", "none", -1, "yes", 2 },
...
function ctld.JTACAutoLase(_jtacGroupName, _laserCode, _smoke, _lock, _colour)
    ...
    if _enemyUnit == nil and ctld.jtacCurrentTargets[_jtacGroupName] ~= nil then
        ...
        if _tempUnit ~= nil and _tempUnit:getLife() > 0 and _tempUnit:isActive() == true then
            if JTAC_sendNotificationMessage then
                ctld.notifyCoalition(_jtacGroupName .. " target " .. _tempUnitInfo.unitType .. " out of sight.", 5, _jtacUnit:getCoalition())
            end
        else
            ctld.notifyCoalition(_jtacGroupName .. " target " .. _tempUnitInfo.unitType .. " destroyed.", 5, _jtacUnit:getCoalition())
        end
    ...
    if _enemyUnit == nil then
        ...
        if _enemyUnit ~= nil then
            ...
            ctld.notifyCoalition(_jtacGroupName .. " lasing new target " .. _enemyUnit:getTypeName(), 5, _jtacUnit:getCoalition())
...
function ctld.notifyCoalition(_message, _displayFor, _side)
    ...
    trigger.action.outSoundForCoalition(_side, "walkietalkie.ogg")
end
...
function ctld.createSmokeMarker(_enemyUnit, _colour)
    ...
    -- EDIT THIS AND MAKE MORE FUZZY
    -- DISPLACE SMOKE MARKER AROUND 150mts or perhaps even more
    -- Perhaps add a random to that, could be something in the range of 50mt to 500mt
    local randomSmokeDisplacement = math.random(250)
    local _enemyPoint = _enemyUnit:getPoint()
    trigger.action.smoke({ x = _enemyPoint.x + randomSmokeDisplacement, y = _enemyPoint.y + 2.0, z = _enemyPoint.z + randomSmokeDisplacement}, _colour)
end
...
function ctld.getJTACStatus(_args)
    ...
    for _jtacGroupName, _jtacDetails in pairs(ctld.jtacUnits) do
        ...
        if _jtacUnit ~= nil and _jtacUnit:getLife() > 0 and _jtacUnit:isActive() == true and _jtacUnit:getCoalition() == _side then
            ...
            if _enemyUnit ~= nil and _enemyUnit:getLife() > 0 and _enemyUnit:isActive() == true then
                _message = _message .. "\n" .. _jtacGroupName .. " targeting " .. _enemyUnit:getTypeName() .. "\n" .. ctld.getPositionString(_enemyUnit) .. "\n"
                ...
                _message = _message .. "LASER CODE " .. _laserCode .. "\n"

            else
                if ctld.JTAC_sendNotificationMessage then
                    _message = _message .. "\n" .. _jtacGroupName .. " searching for targets\n"
                end
            end
        end
    end
    ...
    if _message == "JTAC STATUS \n\n" then
        _message = "No Active JTACs or targets available"
    end


    ctld.notifyCoalition(_message, 60, _side)
end
...
function ctld.isInfantry(_unit)
    ...
    local _soldierType = { "infantry", "paratrooper", "stinger", "manpad", "mortar", "soldier", "igla", "emplacement", "outpost", "watch tower", "house1arm", "house2arm" }
...
function ctld.getPositionString(_unit)
    ...
    local _elevation = land.getHeight({x = _unitPos.x, y = _unitPos.z})
    local _elevationInFeet = math.ceil(_elevation * 3.3)

    local _latLngStr = mist.tostringLL(_lat, _lon, 3, ctld.location_DMS)

    local _mgrsString = mist.tostringMGRS(coord.LLtoMGRS(coord.LOtoLL(_unit:getPosition().p)), 5)

    return "LAT/LONG\t" .. _latLngStr .. "\nMGRS\t" .. _mgrsString .."\nELEV\t" .. _elevationInFeet .."ft"
end
